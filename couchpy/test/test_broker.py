"""
Largely equal to the original code by Christopher Lenz for CouchDB-Python.
"""
...
from couchpy import broker, network
...
class DatabaseTestCase(testutil.TempDatabaseMixin, unittest.TestCase):
    ...
    def test_attachment_crud(self):
        ...
        self.assertEqual('Föö bar'.encode('latin-1'),
                         self.db.getAttachment('foo', 'foo.txt').data)
    ...
    def test_default_attachment(self):
        ...
        sentinel = object()
        self.assertTrue(self.db.getAttachment(doc, 'missing.txt', sentinel) is
                        sentinel)
    ...
    def test_json_attachment(self):
        ...
        self.assertEquals(b'{}', self.db.getAttachment(doc, 'test.json').data)
...
class ViewTestCase(testutil.TempDatabaseMixin, unittest.TestCase):
    ...
    def test_view_multi_get(self):
        ...
        res = list(self.db.view('test/multi_key',
                                doc_ids=list(range(1, 6, 2))))
    ...
    def test_init_with_resource(self):
        self.db['foo'] = {}
        view = broker.PermanentView(self.db.resource('_all_docs').url,
                                    '_all_docs')
    ...
    def test_wrapper_iter(self):
        class Wrapper(object):
            def __init__(self, _):
                pass
        self.db['foo'] = {}
        self.assertTrue(isinstance(list(self.db.view('_all_docs',
                                                     wrapper=Wrapper))[0],
                                   Wrapper))
    ...
    def test_wrapper_rows(self):
        class Wrapper(object):
            def __init__(self, _):
                pass
        self.db['foo'] = {}
        self.assertTrue(isinstance(next(self.db.view('_all_docs',
                                                     wrapper=Wrapper).rows),
                                   Wrapper))

    def test_properties(self):
        for attr in ['rows', 'total_rows', 'offset']:
            self.assertTrue(getattr(self.db.view('_all_docs'), attr) is
                            not None)
...
class ShowListTestCase(testutil.TempDatabaseMixin, unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.db.bulk([{'_id': '1', 'name': 'one'},
                      {'_id': '2', 'name': 'two'}])

    def test_show_urls(self):
        self.assertEqual(b'null:<default>',
                         self.db.show('_design/foo/_show/bar').data)
        self.assertEqual(b'null:<default>', self.db.show('foo/bar').data)

    def test_show_docid(self):
        self.assertEqual(b'null:<default>', self.db.show('foo/bar').data)
        self.assertEqual(b'1:<default>', self.db.show('foo/bar', '1').data)
        self.assertEqual(b'2:<default>', self.db.show('foo/bar', '2').data)

    def test_show_params(self):
        self.assertEqual(b'null:abc', self.db.show('foo/bar', r='abc').data)

    def test_list(self):
        self.assertEqual(str(self.db.list('foo/list', 'foo/by_id').data),
                         '1\r\n2\r\n')
        self.assertEqual(self.db.list('foo/list', 'foo/by_id',
                                      include_header='true').data.read(),
                         b'id\r\n1\r\n2\r\n')

    def test_list_keys(self):
        self.assertEqual(self.db.list('foo/list', 'foo/by_id',
                                      doc_ids=['1']).data.read(), b'1\r\n')

    def test_list_view_params(self):
        self.assertEqual(self.db.list('foo/list', 'foo/by_name',
                                      startkey='o', endkey='p').data.read(),
                         b'1\r\n')
...
class UpdateHandlerTestCase(testutil.TempDatabaseMixin, unittest.TestCase):
    ...
    def setUp(self):
        ...
        # Workaround for possible bug in CouchDB. Adding a timestamp avoids a
        # 409 Conflict error when pushing the same design doc that existed in
        # a now deleted database.
        design_doc = dict(self.design_doc)
        design_doc['timestamp'] = time.time()
        self.db.save(design_doc)
    ...
    def test_empty_doc(self):
        self.assertEqual(self.db.update('foo/bar').data, b'empty doc')

    def test_new_doc(self):
        self.assertEqual(self.db.update('foo/bar', 'new').data, b'new doc')

    def test_update_doc(self):
        self.assertEqual(self.db.update('foo/bar', 'existed').data,
                         b'hello doc')
