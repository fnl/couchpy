...
from couchpy.network import ResourceConflict, ResourceNotFound
...
class CouchTests(testutil.TempDatabaseMixin, unittest.TestCase):
    ...
    def test_design_docs(self):
        ...
        for idx, row in enumerate(self.db.view('test/all_docs')):
            self.assertEqual(idx, row.key)
        self.assertEqual(50, len(list(self.db.view('test/all_docs'))))
    ...
    def test_collation(self):
        ...
        self.assertEqual(len(values), view.total_rows)
...
def suite():
    ...
    suite.addTest(unittest.makeSuite(CouchTests))
    return suite
